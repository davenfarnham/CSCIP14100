...
def extract_names(filename):
  ...
  for line in f:
    ...
    if h3:
        ...
        if names:
          ...
          boy = str((names[1]))[4:]
          girl = str((names[2]))[4:]
          dict[rank] = (boy, girl) # store in dict as rank -> (boy's name, girl's name)
  ...
  for i in items:
    boy, girl = i[1] # match on tuple
    expanded.append(boy + ' ' + i[0]) # put boy's name and rank into list
    expanded.append(girl + ' ' + i[0]) # put girl's name and rank into list
